...
public function assistantCheckoutData(Request $request)
{
    ...
    return response()->json(['payload' => ['deliveryDate' => $selectedSlot['date'], 'delivery_time_slot' => $selectedSlot['label'], 'billingAddress' => $billing, 'shippingAddress' => $shipping, 'subtotal' => $subtotal, 'user_id' => $outlet->id, 'productDiscount' => $discount, 'cgstSgst' => $gst, 'packingCharges' => $packing, 'othersCharges' => $others, 'deliveryCharges' => $deliveryCharge, 'shipping_pincode' => $deliveryKyc?->outlet_pincode, 'totalDiscountValue' => round($subtotal + $gst + $deliveryCharge + $packing + $others - $coupon, 2), 'cart' => $cart->toArray()]]);
}
