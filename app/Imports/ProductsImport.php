...
class ProductsImport implements ToCollection
{
    ...
    private function getOrCreateVendor($name, &$vendorsCache)
{
    if (!$name) return null;
    ...
    if (isset($vendorsCache[$key])) {
        return $vendorsCache[$key];
    }
    ...
    $vendorsCache = Vendor::pluck('id', 'name')
    ...
    $carton_discount_basic = 0; // Default value when total_with_tax is zero
            ...
            $total_discount = ($loose_discount_basic + $carton_discount_basic) / 2;
               ...
               $subcat = Subcategory::where('name', $row[11])->where('category_id', $category->id)->first();
                ...
                $supplierName = trim($row[23]);
                $vendorId = $this->getOrCreateVendor($supplierName, $vendorsCache);
