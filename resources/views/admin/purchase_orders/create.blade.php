                ...
                <div class="row">
                    ...
                    @endif
                        <div class="card">
                            <div class="card-body">
                                ...
                                <div class="col-md-4 mb-3">
                                    ...
                                    <input type="text" id="purchase_order_number" value="{{ $purchaseOrderNumber }}" class="form-control" readonly>
                                </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <h4 class="mb-0">Purchase Products</h4>

                            <a href="{{ route('vendor.price.create') }}" class="btn btn-sm btn-primary">
                                <i class="fa fa-plus me-1"></i> Create Vendor Price List
                            </a>
                        </div>
                         ...
                         <div class="table-responsive">
                                <table class="table table-bordered align-middle" id="invoiceTable">
                                    <thead class="table-light">
                                        <tr>
                                            ...
                                            <th style="width:10%">Vendor Price</th>
                                            <th style="width:10%">Cgst + Sgst</th>
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td>
                                                <input type="text" class="form-control vendor-price" readonly>
                                            </td>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="position-relative">
                                            <input type="number"
                                                id="product_discount"
                                                class="form-control fw-bold text-end pe-4"
                                                placeholder="0">
                                            <span class="position-absolute top-50 end-0 translate-middle-y me-2 fw-bold">%</span>
                                        </div>
                                    </td>
                                ...
                                <tr>
                                    ...
                                    <td>
                                            ...
                                            <input type="text"
                                                id="tax_total_amount"
                                                class="form-control fw-bold text-end tax_total_amount"
                                            readonly value="0.00">
                             ...
                             <div class="row mt-6">
                                <div class="col-md-4 offset-md-8 text-end">
                                    ...
                                    <button type="button"
                                            id="placeOrderBtn"
                                            class="btn btn-lg w-100" style="background: #e97457; color: #fff;">
...
$(document).ready(function () {
    ...
    function loadProducts($select) {
        ...
        if (!vendorId) {
            $select
                .prop('disabled', true)
                .html('<option value="">Select vendor first</option>')
                .val(null);

            initProductSelect2($select, 'Select vendor first');
            return;
        }
    ...
    $('#pay_on_delivery').prop('checked', false);
...
//         row.find('.carton-size').text(`Box: ${qty} | Stock: ${stock}`);
...
$(document).on('change', '.product-select', function () {
    ...
    fetch(`/admin/check-any-rack-allocation`)
        .then(res => res.json())
        .then(data => {
            ...
            if (!data.allocated) {
                Swal.fire({
                    ...
                    if (result.dismiss === Swal.DismissReason.cancel) {
                        window.open(`/admin/rack-receiving/${data.latest_unallocated_id}/create`, '_blank');
                    }
            ...
            fillRowFromProduct(row, $select, selected);
        })
        .catch(() => {
...
function fillRowFromProduct(row, $select, selected) {
    let cost          = parseFloat(selected.data('cost'))         || 0;
    let price         = parseFloat(selected.data('vendor-price')) || 0;
    let taxpercentage = parseFloat(selected.data('gst'))          || 0;
    let qty           = parseFloat(selected.data('qty'))          || 1;
...
$('#placeOrderBtn').on('click', function () {
    ...
    if (!payload.location) {
        Swal.fire('Vendor Required', 'Please select a location', 'warning');
        return;
    }
   
    if (!payload.pincode) {
        Swal.fire('Vendor Required', 'Please select a pincode', 'warning');
        return;
    }

    if (!payload.po_date) {
        Swal.fire('Date Required', 'Please select PO date', 'warning');
        return;
    }
    ...
    if (!payload.payment_term) {
        Swal.fire('Payment Term Required', 'Please select a payment term', 'warning');
        return;
    }
    ...
    .then(res => {
        ...
        if (!res.success) {
            Swal.fire('Failed', res.message || 'Unable to save PO', 'error');
            return;
        }
