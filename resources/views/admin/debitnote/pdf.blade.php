...
@php
$receivingItem = $item->receivingItem; // null for opening stock
$product = $receivingItem->product ?? $item->product ?? null;

$qty = $item->return_qty;
$rate = $item->price;
$amount = $qty * $rate;

// tax already computed & stored on the item at creation time (see $item->tax),
// so recompute the % from what's stored rather than relying on receivingItem
$taxPercent = $receivingItem->row_tax ?? ($amount > 0 ? round(($item->tax / $amount) * 100, 2) : 0);
